...
# STAGING TABLES
staging_events_copy = open('sql/staging_events_copy.sql', 'r').read().format(
    config['S3']['LOG_DATA'], config['IAM_ROLE']['ARN'].replace("'", ""))
staging_songs_copy = open('sql/staging_songs_copy.sql', 'r').read().format(
    config['S3']['SONG_DATA'], config['IAM_ROLE']['ARN'].replace("'", ""))

# FINAL TABLES
songplay_table_insert = open('sql/prod_fact_songplays_insert.sql', 'r').read()
user_table_insert = open('sql/prod_dim_users_insert.sql', 'r').read()
